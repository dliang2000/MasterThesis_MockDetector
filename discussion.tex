%======================================================================
\chapter[Discussion]{Discussion\footnote{The contents of this 
...
\section{Discussion on Doop's Basic-only Base Analysis}

Regarding the fact that the call graph generated by Doop's basic-only base analysis interprocedurally would only report edges that have target methods at the abstract level, ignoring the call-graph edges reaching to the implementation sites, we are in the process of extracting a minimal working example from the investigated benchmarks with the known behaviour, and will approach the doop developers with a sensible question regarding the expected behaviour of basic-only analysis. % Will update this part and third bullet point in Future Work
...
\begin{itemize}
	...
	\item The current interprocedural Doop's implementation could not differentiate method invocations in the test suites from the ones in the source code. We will work on modifying Doop's implementation to only report results from the test suites for move accurate results.
	...
	\item We are working on a minimal working example to illustrate the current CHA behaviour from Doop's basic-only base analysis, and we will ask Doop developers some sensible questions about the base analyses we have used (especially for basic-only), so that we could have better insights of our Doop's results on mock analysis.
\end{itemize}
