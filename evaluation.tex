...
Analyzing the interprocedural results, CI and CIPP report more mock invocations in 7 out of the 9 benchmarks comparing to basic-only's reportings. For benchmarks \textsc{flink-core}, \textsc{jsonschema2pojo-core}, \textsc{mybatis} and \textsc{vraptor-core}, CI and CIPP report significantly more mock invocations than basic-only. Our investigation in Section~\ref{sec:basic-only-vs-others} suggests that such discrepancies is due to basic-only base analysis only reports edges to the abstract method and misses the edges reach to the implementation sites. We need to have more discussions with doop authors before giving a definite conclusion, but we believe basic-only needs improvement before using it to get reliable pointer analysis results.
