...
Table~\ref{tab:field-mocks} displays the number of field mock objects that are defined via \texttt{@Mock} annotations, in the constructors, and in the \texttt{@Before} methods, respectively. 

Benchmarks include jsonschema2pojo, mybatis, and vraptor, which have the number of test-related methods with mock objects (intraprocedural analysis) and number of intraprocedural mock invokes at the high end, also tend to define mock objects in a more systematic way, coming with a large number of field mock objects defined in the pre-settings of each test class, and consequently used in multiple test cases. In addition, benchmarks like bootique and maven-core also prefer to define field mock objects in the pre-settings. Therefore, 5 out of the 8 open-source benchmarks likes to define mock objects in the test suites following a more systematic way, defining the field mock objects at the pre-setting stage. This suggests that our pre-analysis for field mocks described in Section~\ref{subsec:pre-analysis} is indeed a necessary but also an effective step for analyzing mock objects in the test suites.
